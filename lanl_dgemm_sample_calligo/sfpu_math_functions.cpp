...
int main(int argc, char **argv) {
    ...
    try {
        ...
        */
        ...
        constexpr uint32_t num_tiles = 1;//64;
        ...
        /*const std::map<std::string, std::string> sfpu_defines = {
            ...
            {"SFPU_OP_CHAIN_0", "sqrt_tile_init(); sqrt_tile(0);"}
        };*/
        ...
        std::vector<uint32_t> src0_vec(2);
        float val = 4.787657;
        src0_vec[0] = *(uint32_t*)&val;
        ...
        float data_result = *(float*)&result_vec[0];
        float data_result1 = std::exp(val);
        //float data_result1 = std::sqrt(val);

        //printf("Execution is done : Result = %u %f %f\n", result[0], data_result, data_result1);
        printf("Execution is done Metalium : Result[0] = %f %f\n", val, data_result);
        printf("Execution is done Host(Expected)     : Result[1] = %f %f\n", val, data_result1);
